...
def get_new_value(old_gen, old_automata):
    ...
    for i in range(SQ_NUM):
        ...
        if i - 1 >= 0 and old_automata[index - 1]:
            live += 1
        if i + 1 < (SQ_NUM) and old_automata[index + 1]:
            live += 1

        if old_automata[index] and live < 2 and index + SQ_NUM < (SQ_NUM * SQ_NUM):
            old_automata[index + SQ_NUM] = 1
        elif old_automata[index] and live > 1 and index + SQ_NUM < (SQ_NUM * SQ_NUM):
